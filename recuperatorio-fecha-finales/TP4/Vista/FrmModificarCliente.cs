...
namespace Vista
{
    public partial class FrmModificarCliente : Form
    {
        ...
        private bool ValidarCampos()
        {
            ...
            {
                esValido = false;
                str.AppendLine("El nombre (limite de 50 caracteres)");
            }

            if (string.IsNullOrWhiteSpace(txtApellido.Text) || txtApellido.Text.Length > 50)
            {
                esValido = false;
                str.AppendLine("El apellido (limite de 50 caracteres)");
            }
            if (string.IsNullOrWhiteSpace(txtDireccion.Text) || txtDireccion.Text.Length > 50)
            {
                esValido = false;
                str.AppendLine("La direccion (limite de 50 caracteres)");
            }
            if (string.IsNullOrWhiteSpace(txtCorreo.Text) || !this.validarCorreo())
            {
                esValido = false;
                str.AppendLine("El correo electronico (con @ y terminando en .com/limite de 50 caracteres)");
            }
        ...
        {
            bool esValido = false;
            if (txtCorreo.Text.Contains("@") && txtCorreo.Text.EndsWith(".com") && txtCorreo.Text.Length < 50)
            {
                esValido = true;
            }
            return esValido;
